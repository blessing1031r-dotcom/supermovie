...
| 項目 | 値 (Bash 実測 2026-05-05 22:31) |
|---|---|
| HEAD (source commit) | `e7dc2bf` (PR-D fix iter 5 — Codex 22:30 P2×2 解消: regen 動的 branch label + Migration table wording 緩和、anchor drift 1 intrinsic) |
| prev source commits | `86abf83` (fix iter 4) → `b837430` (fix iter 3) → `5a5238c` (fix iter 2) → `6b15953` (fix iter 1) → `b862281` (PR-D rate env v0→v1 alias feat) |
| branch | `roku/obs-rate-alias` (`fork/main=b52fd97` 起点、PR #1-#6 全 merged 後の post-step-3 priority cycle) |
| main..HEAD | 158 commits |
...
| 7 gate composite | ALL PASS at e7dc2bf (env / worktree clean / regen drift 1 / **52/52 python smoke** / lint exit 0 / React 22/22 / **gate 7 anchor drift = 1 intrinsic OK**、Bash 実測 22:31、PR-D fix iter 5 で regen script に動的 branch 検出を追加、source 影響なし) |
