...
| 項目 | 値 (Bash 実測 2026-05-05 22:14) |
|---|---|
| HEAD (source commit) | `5a5238c` (PR-D fix iter 2 — Codex 22:11 verdict P2 #1 解消: OBSERVABILITY.md 後段 stale migration 記述を完了履歴へリフレーム、anchor drift 1 intrinsic) |
| prev source commits | `6b15953` (fix iter 1 — Codex 22:05 P2×2) → `b862281` (PR-D rate env v0→v1 alias feat) |
| branch | `roku/obs-rate-alias` (`fork/main=b52fd97` 起点、PR #1-#6 全 merged 後の post-step-3 priority cycle) |
| main..HEAD | 152 commits |
...
| 7 gate composite | ALL PASS at 5a5238c (env / worktree clean / regen drift 1 / **52/52 python smoke** / lint exit 0 / React 22/22 / **gate 7 anchor drift = 1 intrinsic OK**、Bash 実測 22:14、PR-D fix iter 2 docs-only diff のため source 影響なし) |
