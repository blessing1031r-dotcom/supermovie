...
| 項目 | 値 (Bash 実測 2026-05-05 22:21) |
|---|---|
| HEAD (source commit) | `b837430` (PR-D fix iter 3 — Codex 22:14 P2×2 解消: Scope/Current Surface/Redaction/Path/UserContent/dry-run/Test Requirements 全 stale 記述を完了済 PR pointer へ書換、anchor drift 1 intrinsic) |
| prev source commits | `5a5238c` (fix iter 2 — Codex 22:11 P2 #1 後段 stale 一部解消) → `6b15953` (fix iter 1 — Codex 22:05 P2×2 test+冒頭) → `b862281` (PR-D rate env v0→v1 alias feat) |
| branch | `roku/obs-rate-alias` (`fork/main=b52fd97` 起点、PR #1-#6 全 merged 後の post-step-3 priority cycle) |
| main..HEAD | 154 commits |
