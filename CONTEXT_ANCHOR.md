...
| 項目 | 値 (Bash 実測 2026-05-05 21:36) |
|---|---|
| HEAD (source commit) | `34b5649` (PR-B preflight_video v1 — 既存 stdout source JSON 維持 + --json-log tail emit、anchor 自身の document commit はこの後ろに 1 件積まれる、§Source commit vs Document commit 規約 参照) |
| branch | `roku/obs-prb-preflight` (`fork/main=a6a6cc6` 起点、PR #1+#2+#3+#4 merged 後の next cycle、step 3 part 2/3) |
| main..HEAD | 148 commits |
...
| 7 gate composite | ALL PASS at 34b5649 (env / worktree clean / regen drift 1 / **51/51 python smoke (43 既存 + 8 obs)** / lint exit 0 / React 22/22 / **gate 7 anchor drift = 1 intrinsic OK**、Bash 実測 21:36) |
