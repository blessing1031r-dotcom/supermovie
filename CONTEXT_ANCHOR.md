...
| 項目 | 値 (Bash 実測 2026-05-05 22:25) |
|---|---|
| HEAD (source commit) | `86abf83` (PR-D fix iter 4 — Codex 22:23 P2×3 解消: v0 JSON tail/output gap section を legacy 履歴化、Test Requirements wording 緩和、anchor branch label 更新) |
| prev source commits | `b837430` (fix iter 3) → `5a5238c` (fix iter 2) → `6b15953` (fix iter 1) → `b862281` (PR-D rate env v0→v1 alias feat) |
| branch | `roku/obs-rate-alias` (`fork/main=b52fd97` 起点、PR #1-#6 全 merged 後の post-step-3 priority cycle) |
| main..HEAD | 156 commits |
...
| 7 gate composite | ALL PASS at 86abf83 (env / worktree clean / regen drift 1 / **52/52 python smoke** / lint exit 0 / React 22/22 / **gate 7 anchor drift = 1 intrinsic OK**、Bash 実測 22:25、PR-D fix iter 4 docs-only diff のため source 影響なし) |
