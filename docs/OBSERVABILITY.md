...
| 64 | docs §Stdout And Stderr 表 ↔ caller stream emission pattern presence lint 追加 (`test_observability_stdout_stderr_stream_contract_lint`、Codex 07:28 推奨 BW、PR-BD/BE/BF/BH/BI/BJ/BK 同型を Stdout And Stderr 表に展開)。docs §Stdout And Stderr は v1 emission の 3 stream 役割を表で定義 (`stdout (human)` 進捗 message、`stdout (json tail)` 末尾 1 行 schema v1 JSON、`stderr` error/warning/stack trace)。docs と code が drift すると、(a) human stream を全 json tail 化で consumer の human progress 観測喪失、(b) stderr 行削除で error が stdout に流れ `--json-log` parser 破壊、(c) json tail 行削除で v1 schema 観測完全性崩壊。本 lint は docs heading 直後 markdown table から 3 stream label (`stdout (human)` / `stdout (json tail)` / `stderr`) の docs presence assert (label rename / 表 構造変更 fail-loud) + V1_CALLER_SCRIPTS 7 caller それぞれを `ast.parse()` で AST walk し 3 stream emission primitive を全持ちすることを Call node 検出で audit ((1) `ast.Call(func=Name('print'))` で keyword 'file' 無し = human stdout、(2) `ast.Call(func=Name('emit_json' | '_obs_emit_json'))` の Call node 経由 = json tail (FunctionDef の wrapper 定義は Call node ではないので definition だけ残る drift も検出)、(3) `print(..., file=<word>.stderr)` keyword.value Attribute / `<word>.stderr.write(...)` Call attr で stderr stream、build_telop_data の `_sys.stderr` alias も AST `_is_stderr_value()` で確実 match)。caller の stream 欠落 drift を fail-loud (Codex 07:34/07:36 P2 review で raw regex の `[^)]*file=` greedy 終端 + `def emit_json(` definition false-positive を AST 切替で解消)。PR-BD/BE/BF/BH/BI/BJ/BK 同 level の docs/code 双方向 audit、本 lint は stream 振り分け axis を fix + 1 件 regression test | PR-BL |
