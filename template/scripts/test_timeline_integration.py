...
def test_narration_barrel_public_export_surface_contract_lint() -> None:
    """PR-GW: Narration barrel must expose only the stable narration API."""
    import re

    template_root = Path(__file__).parents[1]
    barrel_path = template_root / "src" / "Narration" / "index.ts"
    assert barrel_path.is_file(), "template/src/Narration/index.ts not found"
    raw = barrel_path.read_text(encoding="utf-8")
    text = "\n".join(line for line in raw.splitlines() if not line.lstrip().startswith("//"))
    text = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    assert not re.search(r"""^\s*export\s+default\b""", text, re.MULTILINE), (
        "template/src/Narration/index.ts must not use a default export"
    )
    expected = {
        ("value", "./NarrationAudio"): {"NarrationAudio", "NarrationAudioWithMode"},
        ("value", "./narrationData"): {"narrationData"},
        (
            "value",
            "./mode",
    ...
    extra_keys = sorted(set(actual) - set(expected))
    if missing_keys:
        errors.append(f"missing Narration barrel export source(s): {missing_keys}")
    if extra_keys:
        errors.append(f"extra Narration barrel export source(s): {extra_keys}")
    for key, names in expected.items():
        exported = actual.get(key, set())
        if exported != names:
            errors.append(
                f"Narration barrel export drift for {key}: "
                f"expected {sorted(names)}, got {sorted(exported)}"
            )
    assert errors == [], (
        "template/src/Narration/index.ts public export surface drift:\n"
        + "\n".join(errors)
    )
...
def test_se_data_public_export_surface_contract_lint() -> None:
    """PR-HB: seData module must export only the seData array."""
    import re
    ...
    text = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    assert not re.search(r"""^\s*export\s+default\b""", text, re.MULTILINE), (
        "template/src/SoundEffects/seData.ts must not use a default export"
    )
    assert not re.search(r"""^\s*export\s*\{""", text, re.MULTILINE), (
        "template/src/SoundEffects/seData.ts must not re-export additional symbols"
    )
    exported = set(
        re.findall(
            r"""^\s*export\s+(?:const|let|var|function|class|type|interface|enum)\s+([A-Za-z_][A-Za-z0-9_]*)\b""",
    ...
    assert exported == {"seData"}, (
        "template/src/SoundEffects/seData.ts public export surface drift: "
        f"expected ['seData'], got {sorted(exported)}"
    )
...
def test_slide_data_public_export_surface_contract_lint() -> None:
    """PR-HF: slideData module must export only the slideData array."""
    import re
    ...
    text = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    assert not re.search(r"""^\s*export\s+default\b""", text, re.MULTILINE), (
        "template/src/Slides/slideData.ts must not use a default export"
    )
    assert not re.search(r"""^\s*export\s*\{""", text, re.MULTILINE), (
        "template/src/Slides/slideData.ts must not re-export additional symbols"
    )
    exported = set(
        re.findall(
            r"""^\s*export\s+(?:const|let|var|function|class|type|interface|enum)\s+([A-Za-z_][A-Za-z0-9_]*)\b""",
    ...
    assert exported == {"slideData"}, (
        "template/src/Slides/slideData.ts public export surface drift: "
        f"expected ['slideData'], got {sorted(exported)}"
    )
...
def test_image_sequence_wraps_image_segments_in_sequence_frame_ranges_lint() -> None:
    ...
    text = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    errors: list[str] = []
    if not re.search(r"\binsertImageData\.map\s*\(", text):
        errors.append("ImageSequence.tsx: insertImageData.map() not found — must be data-driven")
    if not re.search(r"\bkey=\{segment\.id\}", text):
        errors.append("ImageSequence.tsx: key={segment.id} not found on <Sequence>")
    if not re.search(r"\bfrom=\{segment\.startFrame\}", text):
        errors.append("ImageSequence.tsx: from={segment.startFrame} not found on <Sequence>")
    if not re.search(r"\bdurationInFrames=\{segment\.endFrame\s*-\s*segment\.startFrame\}", text):
        errors.append("ImageSequence.tsx: durationInFrames={segment.endFrame - segment.startFrame} not found — Sequence must span exact segment range")
    if not re.search(r"<InsertImage\s+segment=\{segment\}", text):
        errors.append("ImageSequence.tsx: <InsertImage segment={segment} .../> not found — must pass full segment to renderer")
    assert errors == [], (
        "template/src/InsertImage/ImageSequence.tsx Sequence frame range contract drift:\n"
        + "\n".join(errors)
    )
...
def test_image_sequence_public_export_surface_contract_lint() -> None:
    """PR-HT: ImageSequence module must export only the ImageSequence component."""
    import re
    ...
    text = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    assert not re.search(r"""^\s*export\s+default\b""", text, re.MULTILINE), (
        "template/src/InsertImage/ImageSequence.tsx must not use a default export"
    )
    assert not re.search(r"""^\s*export\s*(?:\*|\{)""", text, re.MULTILINE), (
        "template/src/InsertImage/ImageSequence.tsx must not re-export additional symbols"
    )
    exported = set(
        re.findall(
            r"""^\s*export\s+(?:const|let|var|function|class|type|interface|enum)\s+([A-Za-z_][A-Za-z0-9_]*)\b""",
    ...
    expected = {"ImageSequence"}
    assert exported == expected, (
        "template/src/InsertImage/ImageSequence.tsx public export surface drift: "
        f"expected {sorted(expected)}, got {sorted(exported)}"
    )
...
def main() -> int:
    tests = [
        ...
        # PR-GU (Title barrel exports only TitleSequence and TitleSegment): 1 件
        test_title_barrel_public_export_surface_contract_lint,
        # PR-GV (InsertImage barrel exports only renderer, sequence, and type): 1 件
        ...
        # PR-HB (seData module exports only the seData array): 1 件
        test_se_data_public_export_surface_contract_lint,
        ...
        # PR-HT (ImageSequence module exports only the ImageSequence component): 1 件
        test_image_sequence_public_export_surface_contract_lint,
